...
export function getCodexBinaryPath(): CodexBinaryResult | null {
  ...
  console.error(
    "[codex-binary] Install via: npm install -g @openai/codex"
  )
...
export function getCodexOAuthToken(): string | null {
      ...
      try {
        ...
        if (output) {
          ...
          try {
            ...
            const token = credentials?.access_token || credentials?.accessToken || credentials?.api_key
            if (token) {
  ...
  for (const authPath of authPaths) {
    try {
      if (fs.existsSync(authPath)) {
        ...
        if (oauthToken) {
          console.log(`[codex] Found OAuth token in ${path.basename(authPath)}`)
          return oauthToken
        }
      }
    } catch {
  ...
  console.log("[codex] No credentials found. Run 'codex login' or set OPENAI_API_KEY")
  return null
}
