  ...
  private async killSessionWithTimeout(
    ...
    return new Promise<boolean>((resolve) => {
      ...
      const timeouts: {
        ...
        sigkill?: ReturnType<typeof setTimeout>;
      } = {};
      ...
      const cleanup = (success: boolean) => {
        ...
        if (timeouts.sigterm) clearTimeout(timeouts.sigterm);
        if (timeouts.sigkill) clearTimeout(timeouts.sigkill);
        resolve(success);
      };
      ...
      timeouts.sigterm = setTimeout(() => {
        ...
        timeouts.sigkill.unref();
      }, 2000);
      timeouts.sigterm.unref();
