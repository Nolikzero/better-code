...
export function ProjectFileTree() {
  ...
  const flattenedTree = useMemo(() => {
    ...
    const addEntries = (entries: FileTreeEntry[], depth: number) => {
      for (const entry of entries) {
        ...
        // If folder is expanded and has cached children, add them
        if (entry.type === "folder" && isExpanded && cachedChildren) {
          addEntries(cachedChildren, depth + 1);
        }
      }
    };

    addEntries(rootEntries, 0);
    return result;
