...
export function SubChatSelector({
  ...
  // @ts-expect-error - Mock api type doesn't fully match expected type
  const renameMutation = api.agents.renameSubChat.useMutation({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    onSuccess: (_: any, variables: { subChatId: string; name: string } | null) => {
      if (variables) {
        // Update local store
        useAgentSubChatStore
          .getState()
  ...
  const handleEditSave = useCallback(
    async (subChat: SubChatMeta) => {
      ...
      useAgentSubChatStore
        .getState()
        .updateSubChatName(subChat.id, trimmedName);
